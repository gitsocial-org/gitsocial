...
The guarded tests call `fullTierOnly`: the TUI matrices `TestSmoke`, `TestSequence` and `TestGolden/LayoutProperties`, the CLI `--json` walk `TestCommandTreeJSONOutput`, the `TestS3Helper_*` child-process tests, and the site fixture build `TestSiteFixtureBuild`. The fixture build needs neither node nor Chrome. It reruns when the site assets, the site generators or `sitetest/fixture.sh` change. `-race` and the browser site battery run at release from `scripts/release.sh`. `-short` skips the real-git subtests. It is a local smoke run, not a tier.
