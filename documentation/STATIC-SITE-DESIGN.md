...
| Palette | `--bg`, `--text`, `--link`, `--lift`, `--lift-hover`, from the light set `--pl-*` or the dark set `--pd-*`; file-type hues `--i-*` | no hex outside `:root` |
| State colors | `--open` #1f9d55, `--closed` #8957e5, `--merged` #8250df, `--warn` #bf8700, `--danger` #cf222e | one meaning per color, everywhere |
| Derived | `--muted`, `--line`, `--panel`, `--chip`, `--chip-hover`, `--code-bg`, `--btn`; tints `--link-t1..3`, `--open-t1..3`, `--warn-t1..3`, `--danger-t1..3` | computed with `color-mix` on `body`, never restated |
...
A card is a mix off the page like every other surface: `--chip` is white at `--lift`, the card is `--chip`, and its hover is `--chip-hover`, white at `--lift-hover`; the theme gates set the pair, 35% and 70% in light, where it lifts the parchment toward cream, and 6% and 12% in dark, where the ground is already near black. The one darker fill, `--code-bg`, is a text mix, because white cannot darken.
...
| Release notes | `dl.release-notes` with a `dt` and a `dd` per commit | both | a release body block whose every line reads `<hash> <message>` renders as these rows: the hash a mono link to its commit route, the message beside it; they take the trailer layout and its mono voice, and every other block of the body stays prose |
...
| Quoted context | `.reply-quote` on a card, `.embedded` on a detail page, each a lead line, the excerpt and the author | app | one kind, so one marker: `--code-bg` behind a 5px left border; a card's excerpt is clickable and opens the parent, and a detail's leads with "from <url>" for a cross-repo reference or "in reply to <ref>" for a parent the walk missed |
...
| Markdown | `.markdown`, headings with `md-` ids, lists, tables, fences, images, blockquotes | both | one grammar, ported between JS and Go, asserted equal; a fence pads `--sp-2` over `--sp-3`, so the first line's leading evens the two |
...
| Stat tile | `.stat-cell` holding `.stat-value` or `.stat-value-text`, then `.stat-label` | app | all three are mono: a figure is a count, not prose |
...
The tokens, the components and their states are checked by `verify_styles.js`, one of the [browser suites](STATIC-SITE.md#testing). Its baselines record a probed element's font family, so a component that drops out of the mono voice fails the suite. Both suites need Chrome.
