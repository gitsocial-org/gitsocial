...
The reader fetches git objects straight from the bucket, which stores them either as loose keys or inside [packfiles](S3.md#packfiles), never both for the same object. `getObject` handles both: it tries the loose key, falls back to the pack path, and once a packed read has succeeded it flips the order for the rest of the session so a pack-only bucket never pays a 404 per object.

Two pack paths, split by cost:

- **Commits and tags** (every item detail, and every code commit, since the items index carries the subject but not the body) resolve through the pack map, `.gitsocial/packmap/<xx>.json`: one shard per two-hex sha prefix, giving `[packIndex, offset, size]`. The read is a single Range GET of a self-contained zlib stream, since the commits pack is written with `--depth=0` and has no deltas. Cold, a detail route pays one shard plus one range where an all-loose bucket paid one object GET; a shard covers 1/256 of history and is cached for the session, so the amortized cost is lower. `verify_request_budget.js` ceilings are unchanged, and `verify_packfiles.js` pins the packed detail route against the same ceiling.
- **Trees and blobs** (file views, diffs) have no map entry: the reader fetches `objects/pack/<name>.idx`, parses it (v2 fanout, sorted sha table, 4-byte offsets with the 8-byte large-offset table), binary-searches the sha, Range GETs the entry, and resolves `OFS_DELTA` / `REF_DELTA` chains itself. Indexes and pack sizes are cached per session, so this is paid once and never on the item hot path.
