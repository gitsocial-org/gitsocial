...
`seedMemoTiersPanic` builds the memo tiers that live outside the workspace repo: the personal bare repo, the session `tui-fixture`, and the repository behind the inherits ref. Each carries one memo, written through the memo API. `TestMain` pins `MEMO_SESSION_ID` to that session, and the repos land under the isolated `HOME`. They outlive one fixture, so setup keeps an existing repo and only re-indexes it. Their commit dates are pinned `fixtureCommitAge` ago: the session picker reads its age from the repo, not the cache.
...
- Horizontal overflow is caught by `assertFrameFits` on every route in `TestGolden/LayoutProperties`, at 120x40, 80x24 and 200x60, and by `assertFitsTerminal` on every `TestDisplay` view. Both measure printable cells with `tuicore.AnsiWidth`, so a border past the terminal edge fails on its own line, not as a golden diff.
- Not caught: a terminal under 80 columns, a view reached by a key rather than a route (a form, a diff, a confirm), and content the frame cut to fit. A clipped line fits, so no test names what it dropped.
