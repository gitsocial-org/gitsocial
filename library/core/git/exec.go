...
func ExecGitTransferContext(ctx context.Context, workdir string, args []string, w io.Writer, extraEnv ...string) (*ExecResult, error) {
	...
	if err != nil {
		...
		log.Debug("git transfer failed", "args", args, "duration_ms", duration.Milliseconds(), "exit_code", exitCode)
		return nil, &GitError{
			Op:     "git " + strings.Join(args, " "),
			Args:   args,
			Err:    ErrGitExec,
	...
	log.Debug("git transfer", "args", args, "duration_ms", duration.Milliseconds())
	return &ExecResult{Stdout: strings.TrimSpace(stdout.String()), Stderr: strings.TrimSpace(stderr.String())}, nil
}
