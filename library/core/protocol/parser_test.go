...
func TestParseMessage_noClosingSeparator(t *testing.T) {
	// Header without trailing " ---" - regex won't match
	input := "Hello\n\n" + `--- GitMsg: ext="social"; type="post"; v="0.1.0"`
	got := ParseMessage(input)
	if got != nil {
		t.Errorf("ParseMessage() should return nil for header without closing separator")
	}
}

func TestParseMessage_headerOnly(t *testing.T) {
	// Just header, no content before it
	input := `--- GitMsg: ext="social"; type="post"; v="0.1.0" ---`
	got := ParseMessage(input)
	if got == nil {
		t.Fatal("ParseMessage() = nil, want non-nil")
...
func TestParseRefSection_malformedFields(t *testing.T) {
	// Ref section with fields that can't be parsed (no = sign, no quotes)
	input := `--- GitMsg-Ref: ext social author Alice; v="0.1.0" ---`
	got := ParseRefSection(input)
	// Should return nil because required fields (ext, author, email, time, ref) won't be extracted
	if got != nil {
