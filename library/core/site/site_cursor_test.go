...
// manifest.
func shardKeySet(t *testing.T, client *objstore.Client) []string {
	t.Helper()
		...
		withTestWalkBudget(5, func() {
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					if tc.rewindItemsMF {
						if err := objstore.PutCompressed(client, siteItemsManifestKey("social"), preItems, ""); err != nil {
							t.Fatalf("rewind items manifest: %v", err)
						}
					}
					if tc.rewindBodiesMF {
						if err := objstore.PutCompressed(client, bodiesManifestKey("social"), preBodies, ""); err != nil {
							t.Fatalf("rewind bodies manifest: %v", err)
						}
					}
					if err := objstore.PutCompressed(client, siteItemsCursorKey("social"), preCursor, ""); err != nil {
