...
func seedCoreConfigCommit(t *testing.T, client *objstore.Client, jsonMsg string) string {
	t.Helper()
...
func TestWriteSiteCustomization(t *testing.T) {
	t.Run("valid site config publishes the artifact", func(t *testing.T) {
		...
		if err := writeSiteCustomization(client, "", refs, objstore.SiteOverride{}, nil); err != nil {
	...
	t.Run("absent ref deletes/skips the artifact", func(t *testing.T) {
		...
		if err := writeSiteCustomization(client, "", map[string]string{}, objstore.SiteOverride{}, nil); err != nil {
...
func TestSiteCustomizationCacheControl(t *testing.T) {
	if got := siteCacheControl(siteCustomizationKey); got != "" {
		t.Fatalf("site-config.json cache class = %q, want the key default (no-cache)", got)
	}
}
