...
func seedGitChain(t *testing.T, client *objstore.Client, n int) (gitDir string, shas []string) {
	t.Helper()
	dir := t.TempDir()
...
func TestLocalWalk_ReadsLocalNotBucket(t *testing.T) {
	...
	src := objstore.NewLocalCommitSource(gitDir, "")
	if src == nil {
		t.Fatal("objstore.NewLocalCommitSource returned nil for a real repo")
	}
	defer src.Close()
