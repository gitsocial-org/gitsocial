...
func TestApplySiteOverride(t *testing.T) {
	...
	t.Run("url override normalizes and wins", func(t *testing.T) {
		got, ok := applySiteOverride(base, true, objstore.SiteOverride{URL: "https://r2.example"})
	...
	t.Run("publish override off disables", func(t *testing.T) {
		got, ok := applySiteOverride(base, true, objstore.SiteOverride{Publish: "false"})
	...
	t.Run("pages override off", func(t *testing.T) {
		got, _ := applySiteOverride(base, true, objstore.SiteOverride{Pages: "false"})
		if got.Pages != "false" {
	...
	t.Run("invalid url override is ignored", func(t *testing.T) {
		got, _ := applySiteOverride(base, true, objstore.SiteOverride{URL: "ftp://nope"})
...
func TestReadSiteCustomization_override(t *testing.T) {
	...
	got, ok, err := readSiteCustomization(client, "", refs, objstore.SiteOverride{URL: "https://r2.example"}, nil)
	if err != nil || !ok {
...
func TestWriteSiteCustomization_overrideStamped(t *testing.T) {
	...
	if err := writeSiteCustomization(client, "", refs, objstore.SiteOverride{URL: "https://r2.example"}, nil); err != nil {
