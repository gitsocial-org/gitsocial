...
func buildCodePages(t *testing.T, client *objstore.Client) (pending bool, state string) {
	...
	pending, state, err := rebuildSitePages(client, "", pagesRefs(client, t), defaultBranch, nil, nil, objstore.SiteOverride{})
	if err != nil {
...
func seedCodeBranch(t *testing.T, client *objstore.Client, parent, salt string, n int) []string {
	t.Helper()
...
func commitsState(t *testing.T, client *objstore.Client) siteCommitsState {
	t.Helper()
...
func TestSiteCommits_SealedLayoutAndRows(t *testing.T) {
	...
	if siteCacheControl("commits/1.html") != "" {
		t.Error("sealed commits pages must revalidate, not cache immutably")
	}
	if siteCacheControl("posts/1.html") != objstore.CacheControlImmutable {
