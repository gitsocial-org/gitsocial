...
func buildFilePages(t *testing.T, client *objstore.Client, dir, head string, site map[string]any) bool {
	t.Helper()
	...
	pending, _, err := rebuildSitePages(client, "", pagesRefs(client, t), "main", src, nil, objstore.SiteOverride{})
	if err != nil {
...
func TestSiteFilePages_UnreadableTree(t *testing.T) {
	...
	if pending, _, err := rebuildSitePages(client, "", pagesRefs(client, t), "main", nil, nil, objstore.SiteOverride{}); err != nil || pending {
