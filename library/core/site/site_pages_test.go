...
func makePageCommit(t *testing.T, parent, author, email, message string, ts int64) (string, []byte) {
	...
	loose, err := objstore.EncodeLooseObject("commit", []byte(body))
	if err != nil {
...
func seedExtMessages(t *testing.T, client *objstore.Client, ext, parent string, specs []pageMsgSpec) []string {
	t.Helper()
...
func seedSocialMessages(t *testing.T, client *objstore.Client, parent string, specs []pageMsgSpec) []string {
	t.Helper()
...
func seedPagesConfig(t *testing.T, client *objstore.Client, site map[string]any) string {
	t.Helper()
...
func pagesRefs(client *objstore.Client, t *testing.T) map[string]string {
	t.Helper()
...
func buildPages(t *testing.T, client *objstore.Client) (pending bool, state string) {
	...
	pending, state, err := rebuildSitePages(client, "", pagesRefs(client, t), "", nil, nil, objstore.SiteOverride{})
	if err != nil {
...
func getKey(t *testing.T, client *objstore.Client, key string) string {
	t.Helper()
	data, err := client.Get(key)
	if err != nil {
		t.Fatalf("get %s: %v", key, err)
	}
	if raw, decErr := objstore.BrotliDecompress(data); decErr == nil && len(raw) > 0 {
		return string(raw)
	}
	return string(data)
}
...
func keyExists(client *objstore.Client, key string) bool {
...
func TestSitePages_SitemapCoverageAndIndexMode(t *testing.T) {
	...
	if siteCacheControl("sitemap-1.xml") != objstore.CacheControlImmutable {
...
func TestSitePages_OldMarkerDoesNotMaskPagesBootstrap(t *testing.T) {
	...
	digest, err := objstore.RefsHeadDigest(client, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSitePages_FrontHome(t *testing.T) {
	...
	src := objstore.NewLocalCommitSource("", dir)
	defer src.Close()
...
// guardOutcome builds the post-push hook's input for a push that moved the social branch to tip.
func guardOutcome(t *testing.T, client *objstore.Client, tip string) objstore.PushOutcome {
	t.Helper()
	return objstore.PushOutcome{
		Client:     client,
		Refs:       pagesRefs(client, t),
		Updates:    map[string]string{"refs/heads/gitmsg/social": tip},
		ManifestOK: true,
	}
}
...
func TestPostPushMaintenance_PublishGuard(t *testing.T) {
	...
	PostPushMaintenance(guardOutcome(t, client, shas[0]))
	for _, key := range []string{siteVersionKey, "index.html", siteItemsManifestKey("social")} {
		if keyExists(client, key) {
			t.Errorf("guard off: the hook must not write %s", key)
		}
	}
	...
	PostPushMaintenance(guardOutcome(t, client2, shas2[0]))
	for _, key := range []string{siteVersionKey, "index.html", siteItemsManifestKey("social")} {
		if !keyExists(client2, key) {
			t.Errorf("guard on: the hook must write %s", key)
		}
	}
}
...
func TestSitePageIcon(t *testing.T) {
	shell, err := siteFiles.ReadFile("assets/index.html")
	if err != nil {
