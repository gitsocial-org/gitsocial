...
func foldSiteOverride(digest string, ov objstore.SiteOverride) string {
...
func readSitePushState(client *objstore.Client, prefix string) (sitePushState, bool) {
...
func siteMaintenanceUpToDate(client *objstore.Client, prefix, shellVersion string, ov objstore.SiteOverride) (upToDate bool, digest string) {
	state, ok := readSitePushState(client, prefix)
	digest, err := objstore.RefsHeadDigest(client, prefix)
	if err != nil {
		return false, ""
	}
...
func writeSitePushState(client *objstore.Client, prefix, shellVersion, digest, pagesState string) {
