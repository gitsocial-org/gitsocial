...
// distinct.
func seedChain(t *testing.T, client *objstore.Client, parent, salt string, n int) []string {
	t.Helper()
...
func corpusShas(t *testing.T, client *objstore.Client, m *siteShardManifest, dir func(string) string, headKey func(string) string, ext string) []string {
	t.Helper()
	var shas []string
...
func mustUpdate(t *testing.T, client *objstore.Client, ext, tip string) {
	t.Helper()
...
func rawDoc(t *testing.T, client *objstore.Client, key string) []byte {
	t.Helper()
...
func TestRepair_BodiesManifestAhead(t *testing.T) {
	withTestShardCount(func() {
		...
		mustUpdate(t, client, "social", shas[6])
		if err := objstore.PutCompressed(client, siteItemsManifestKey("social"), oldManifest, ""); err != nil {
			t.Fatalf("rewind items manifest: %v", err)
		}
		if err := objstore.PutCompressed(client, siteItemsHeadKey("social"), oldHead, ""); err != nil {
...
func TestRepair_ItemsManifestAhead(t *testing.T) {
	withTestShardCount(func() {
		...
		shas = append(shas, seedChain(t, client, shas[4], "", 2)...)
		mustUpdate(t, client, "social", shas[6])
		if err := objstore.PutCompressed(client, bodiesManifestKey("social"), oldManifest, ""); err != nil {
			t.Fatalf("rewind bodies manifest: %v", err)
		}
		if err := objstore.PutCompressed(client, bodiesHeadKey("social"), oldHead, ""); err != nil {
...
func TestRepair_HeadCountMismatch(t *testing.T) {
	withTestShardCount(func() {
		for _, corrupt := range []struct {
			...
			comp, err := objstore.CompressJSON(corrupt.doc(shas[5]), objstore.BrotliQualityFull)
			if err != nil {
				t.Fatalf("%s: compress: %v", corrupt.name, err)
			}
			if err := objstore.PutCompressed(client, corrupt.key, comp, ""); err != nil {
