...
func putShardDoc[E shardEntry](client *objstore.Client, corpus shardCorpus[E], ext, key, tip string, entries []E, quality int) (int, error) {
	comp, err := objstore.CompressJSON(corpus.marshalDoc(ext, tip, entries), quality)
	if err != nil {
		return 0, err
	}
	if err := objstore.PutCompressed(client, key, comp, siteCacheControl(key)); err != nil {
		return 0, err
	}
	return len(comp), nil
}
...
func sealShardGeneric[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, group []E, sizeByHash map[string]int) (siteShardEntry, int, error) {
	...
	if !exists {
		if size, err = putShardDoc(client, corpus, ext, key, endTip, group, objstore.BrotliQualityShard); err != nil {
			return siteShardEntry{}, 0, err
		}
	} else if size == 0 {
...
func putShardManifest[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, m *siteShardManifest) error {
	comp, err := objstore.CompressJSON(m, objstore.BrotliQualityFull)
	if err != nil {
		return err
	}
	return objstore.PutCompressed(client, prefix+corpus.manifestKey(ext), comp, "")
}
...
func planSharded[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, entries []E, sizeByHash map[string]int, sp *siteProgress) (shardPlan[E], error) {
...
func planAppend[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, gap, headItems []E, manifest *siteShardManifest, sp *siteProgress) (shardPlan[E], error) {
...
func planTail[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, keptShards []siteShardEntry, tail []E, sp *siteProgress) (shardPlan[E], error) {
...
func sealSegment[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, segment []E, sizeByHash map[string]int, sp *siteProgress) ([]siteShardEntry, int, error) {
...
func prependSegmentPlan[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string, segment, head []E, sp *siteProgress) (shardPlan[E], string, error) {
...
func putHead[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext, tip string, plan *shardPlan[E]) error {
	headBytes, err := putShardDoc(client, corpus, ext, prefix+corpus.headKey(ext), tip, plan.head, objstore.BrotliQualityFull)
	if err != nil {
		return err
	}
...
func putManifest[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext, tip string, plan shardPlan[E], bodiesBytes int, complete bool) (int, error) {
...
func readDocItems[E shardEntry](client *objstore.Client, key string) ([]E, error) {
	var doc struct {
		Items []E `json:"items"`
	}
	found, err := objstore.ReadCompressedJSON(client, key, &doc)
	if err != nil || !found {
		return nil, err
	}
...
func readShardManifest[E shardEntry](client *objstore.Client, corpus shardCorpus[E], prefix, ext string) (*siteShardManifest, error) {
	var m siteShardManifest
	found, err := objstore.ReadCompressedJSON(client, prefix+corpus.manifestKey(ext), &m)
	if err != nil {
		return nil, err
	}
