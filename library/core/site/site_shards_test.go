...
func testClient(t *testing.T) (*objstore.Client, *membucket.Bucket) {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("objstore.NewClient: %v", err)
	}
...
func writeCorpus[E shardEntry](client *objstore.Client, corpus shardCorpus[E], ext, tip string, entries []E, bodiesBytes int) (int, error) {
...
func TestReadCompressedJSON_TranscodedBody(t *testing.T) {
	...
	plain, err := objstore.BrotliDecompress(compressed)
	if err != nil {
