// site_walk_progress_test.go - the site index walk reports a PLAIN COUNT (never a
...
// formatWalkLine renders one progress call the way the stderr writer does, so the assertions read what a user sees.
func formatWalkLine(phase string, done, total int) string {
	if total <= 0 {
		return fmt.Sprintf("%s: %d", phase, done)
	}
	return fmt.Sprintf("%s: %d/%d (%d%%)", phase, done, total, done*100/total)
}
...
func captureWalkPhase(fn func(progress objstore.Progress)) []string {
	var lines []string
	fn(func(phase string, done, total int) {
		if strings.HasPrefix(phase, "site index ") {
			lines = append(lines, formatWalkLine(phase, done, total))
		}
	})
	return lines
}
...
func TestBootstrapWalk_PlainCount(t *testing.T) {
	...
	withTestShardCount(func() {
		lines = captureWalkPhase(func(progress objstore.Progress) {
