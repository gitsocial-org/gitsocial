...
func ApplySuggestion(workdir string, feedback Feedback) Result[string] {
	if !feedback.Suggestion {
		return result.Err[string]("NOT_SUGGESTION", "feedback is not a suggestion")
	}
	if feedback.File == "" {
		return result.Err[string]("NOT_SUGGESTION", "feedback has no file reference")
	}
	...
	if startLine <= 0 {
		return result.Err[string]("NOT_SUGGESTION", "feedback has no line reference")
	}
	...
	if filepath.IsAbs(clean) || strings.HasPrefix(clean, "..") {
		return result.Err[string]("INVALID_PATH", "file path must be relative and within the repository")
	}
