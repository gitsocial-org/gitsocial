	...
	sections = append(sections, tuicore.Section{
		Items: []tuicore.SectionItem{{
			Render: func(width int, selected bool, searchQuery string, anchors *tuicore.AnchorCollector) []string {
				lines := renderReleaseCard(rel, width, selected, searchQuery, anchors, releaseCardOptions{
					showRaw:       v.showRaw,
					showEmail:     v.showEmail,
					artifactInfos: v.artifactInfos,
					sbomSummary:   v.sbomSummary,
					sbomLoading:   v.sbomLoading,
				})
...
// releaseCardOptions carries view state (show-raw/show-email, live artifact/SBOM
// enrichment) plus version-mode settings into the shared release hero-card renderer.
type releaseCardOptions struct {
	showRaw       bool
	showEmail     bool
	artifactInfos map[string]release.ArtifactInfo
	sbomSummary   *release.SBOMSummary
...
// renderReleaseCard renders the release hero card shared by the detail view and
// the version picker. In version mode it shows an edit banner and suppresses
// current-canonical chrome (origin rows). Live artifact sizes and the SBOM
// summary degrade automatically because version mode passes none.
func renderReleaseCard(rel *release.Release, width int, selected bool, searchQuery string, anchors *tuicore.AnchorCollector, opts releaseCardOptions) []string {
	var lines []string
	selectionBar := " "
	if selected {
	...
	if opts.version {
		lines = append(lines, tuicore.RenderVersionBanner(selectionBar, opts.versionAuthor, opts.versionTime, rel.IsRetracted)...)
	}
	title := rel.Subject
	if searchQuery != "" {
	...
	if !opts.version {
		lines = append(lines, tuicore.RenderOriginRows(rel.Origin, styles, selectionBar, anchors, opts.showEmail)...)
	}
	if rel.Prerelease {
	...
	if len(rel.Artifacts) > 0 {
		for i, a := range rel.Artifacts {
			...
			if info, ok := opts.artifactInfos[a]; ok && info.Size > 0 {
