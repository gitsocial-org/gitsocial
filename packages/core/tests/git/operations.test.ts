...
import * as opsModule from '../../src/git/operations';
...
describe('git/operations', () => {
  ...
  describe('createCommitOnBranch()', () => {
    it('should create commit on new branch', async () => {
      const result = await opsModule.createCommitOnBranch(testRepo.path, 'new-branch', 'Test commit');
      expect(result.success).toBe(true);
      expect(result.data).toBeDefined();
      expect(result.data?.length).toBe(12);
    });
    ...
    it('should create commit on existing branch with parent', async () => {
      ...
      const result = await opsModule.createCommitOnBranch(testRepo.path, 'existing', 'Second commit');
      expect(result.success).toBe(true);
      expect(result.data).toBeDefined();
    });

    it('should return short hash', async () => {
      const result = await opsModule.createCommitOnBranch(testRepo.path, 'test', 'Message');
      expect(result.success).toBe(true);
      expect(result.data?.length).toBe(12);
    });
    ...
    it('should return fallback error when commit-tree returns empty data on existing branch', async () => {
      ...
      const result = await opsModule.createCommitOnBranch(testRepo.path, 'existing', 'Test');
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('GIT_COMMIT_ERROR');
      expect(result.error?.message).toBe('Failed to create commit');
      spy.mockRestore();
    ...
    it('should return fallback error when commit-tree returns empty data on new branch', async () => {
      ...
      const result = await opsModule.createCommitOnBranch(testRepo.path, 'new-branch', 'Test');
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('GIT_COMMIT_ERROR');
  ...
  describe('getConfiguredBranch()', () => {
    it('should return branch from refs/gitmsg/social/config', async () => {
      ...
      const branch = await opsModule.getConfiguredBranch(testRepo.path);
  ...
  describe('validatePushPreconditions()', () => {
    it('should fail on detached HEAD', async () => {
      ...
      const result = await opsModule.validatePushPreconditions(testRepo.path);
      expect(result.success).toBe(false);
    ...
    it('should fail when branch does not exist', async () => {
      ...
      await remotesModule.addRemote(newRepo.path, 'origin', 'https://github.com/user/repo.git');
      const result = await opsModule.validatePushPreconditions(newRepo.path);
      expect(result.success).toBe(false);
    ...
    it('should fail when branches have diverged', async () => {
      await remotesModule.addRemote(testRepo.path, 'origin', 'https://github.com/user/repo.git');
  ...
  describe('mergeBranch()', () => {
    it('should merge branch successfully', async () => {
      ...
      await createTestCommit(testRepo.path, 'Feature commit', { allowEmpty: true });
      await execGit(testRepo.path, ['checkout', 'main']);
      const result = await opsModule.mergeBranch(testRepo.path, 'feature');
      expect(result.success).toBe(true);
    });

    it('should return error on merge failure', async () => {
      const result = await opsModule.mergeBranch(testRepo.path, 'nonexistent');
      expect(result.success).toBe(false);
  ...
  describe('setUpstreamBranch() and getUpstreamBranch()', () => {
    it('should set and get upstream branch', async () => {
      await remotesModule.addRemote(testRepo.path, 'origin', 'https://github.com/user/repo.git');
      await execGit(testRepo.path, ['update-ref', 'refs/remotes/origin/main', 'HEAD']);
      const setResult = await opsModule.setUpstreamBranch(testRepo.path, 'origin/main');
      expect(setResult.success).toBe(true);
      const getResult = await opsModule.getUpstreamBranch(testRepo.path);
    ...
    it('should set upstream for specific branch', async () => {
      ...
      const result = await opsModule.setUpstreamBranch(testRepo.path, 'origin/feature', 'feature');
      expect(result.success).toBe(true);
    });

    it('should fail to get upstream when not set', async () => {
      const result = await opsModule.getUpstreamBranch(testRepo.path);
      expect(result.success).toBe(false);
    });

    it('should fail to set upstream with invalid ref', async () => {
      const result = await opsModule.setUpstreamBranch(testRepo.path, 'origin/nonexistent');
      expect(result.success).toBe(false);
    ...
    it('should get upstream for specific branch', async () => {
      ...
      await execGit(testRepo.path, ['branch', 'feature']);
      await execGit(testRepo.path, ['update-ref', 'refs/remotes/origin/feature', 'HEAD']);
      await opsModule.setUpstreamBranch(testRepo.path, 'origin/feature', 'feature');
      const result = await opsModule.getUpstreamBranch(testRepo.path, 'feature');
      expect(result.success).toBe(true);
