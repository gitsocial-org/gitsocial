...
import { log } from '../../src/social/log';
import { createCommit, createCommitWithTimestamp, createTestRepo, type TestRepo } from '../test-utils';
...
import { interaction } from '../../src/social/post/interaction';
import { cache, postIndex, postsCache } from '../../src/social/post/cache';
  ...
  describe('getLogs()', () => {
    ...
    describe('filters', () => {
      beforeEach(async () => {
        ...
        await createCommitWithTimestamp(testRepo.path, 'Post 1', baseTime, { allowEmpty: true });
        await createCommitWithTimestamp(testRepo.path, 'Post 2', new Date(baseTime.getTime() + 1000), { allowEmpty: true });
        await createCommitWithTimestamp(testRepo.path, 'Post 3', new Date(baseTime.getTime() + 2000), { allowEmpty: true });
      });
